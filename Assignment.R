...
ggsave("Question2.bmp", width=5, height=2)
...
ggsave("Question3.bmp", width=5, height=2)
...
ggsave("Question4.bmp", width=5, height=2)
...
df_summary <- summarize(df_grouped, Steps=sum(steps))
print(ggplot(df_summary, aes(x=by, y=Steps))+geom_bar(stat="identity")+xlab("Date")+ggtitle("Plot of Number of Steps per Date"))
ggsave("Question7.bmp", width=5, height=2)
...
ggsave("Question8.bmp", width=5, height=2)
